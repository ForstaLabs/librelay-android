...
import io.forsta.librelay.media.RoundedCorners;
...
public class ThumbnailView extends FrameLayout {
  ...
  public void setImageResource(@NonNull Slide slide, boolean showControls) {
    if (showControls) {
    ...
    if      (slide.getThumbnailUri() != null) buildThumbnailGlideRequest(slide).into(image);
  ...
  public void setImageResource(@NonNull Uri uri) {
    ...
    Glide.with(getContext()).load(new MediaStreamUriLoader.AttachmentUri(uri))
         .crossFade()
  ...
  private GenericRequestBuilder buildThumbnailGlideRequest(@NonNull Slide slide) {
    @SuppressWarnings("ConstantConditions")
    DrawableRequestBuilder<MediaStreamUriLoader.AttachmentUri> builder = Glide.with(getContext()).load(new MediaStreamUriLoader.AttachmentUri(slide.getThumbnailUri()))
                                                                             .crossFade()
                                                                             .transform(new RoundedCorners(getContext(), true, radius, backgroundColorHint));
