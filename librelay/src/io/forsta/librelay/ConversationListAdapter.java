...
public class ConversationListAdapter extends CursorRecyclerViewAdapter<ConversationListAdapter.ViewHolder> {
  ...
  public long getItemId(@NonNull Cursor cursor) {
    final long id = cursor.getLong(cursor.getColumnIndexOrThrow(ThreadDatabase.ID));
    return id;
  }
                                 ...
                                 @Nullable ItemClickListener clickListener)
  {
    ...
    try {
      this.threadDatabase = DbFactory.getThreadDatabase(context);
      this.locale         = locale;
  ...
  public void onBindItemViewHolder(ViewHolder viewHolder, @NonNull Cursor cursor) {
    viewHolder.getItem().bind(getThreadRecord(cursor), locale, batchSet, batchMode);
  }
